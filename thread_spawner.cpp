...
void ThreadSpawner::spawnProducersThreads(Storage &storage,
        Counter &counter) {
    this->spawnCooks(storage, counter);
    this->spawnCarpenters(storage, counter);
    this->spawnGunSmiths(storage, counter);
}

void ThreadSpawner::spawnWorkersThreads(Storage &storage,
    ...
    for (int i = 0; i < n_farmers; ++i) {
        gatherers.emplace_back(wheat_source, storage);
    }
    for (int i = 0; i < n_woodcutters; ++i) {
        gatherers.emplace_back(wood_source, storage);
    }
    for (int i = 0; i < n_miners; ++i) {
        gatherers.emplace_back(iron_and_carbon_source, storage);
    }
...
void ThreadSpawner::spawnCooks(Storage &storage, Counter &counter) {
    int n_cook = workers["Cocineros"];

    cooks.reserve(n_cook);
    for (int i = 0; i < n_cook; ++i) {
        cooks.emplace_back(storage, counter);
    }
...
void ThreadSpawner::spawnCarpenters(Storage &storage, Counter &counter) {
    int n_carpenter = workers["Carpinteros"];
    carpenters.reserve(n_carpenter);
    for (int i = 0; i < n_carpenter; ++i) {
        carpenters.emplace_back(storage, counter);
    }
...
void ThreadSpawner::spawnGunSmiths(Storage &storage, Counter &counter) {
    int n_gunsmiths = workers["Armeros"];
    gunsmiths.reserve(n_gunsmiths);
    for (int i = 0; i < n_gunsmiths; ++i) {
        this->gunsmiths.emplace_back(storage, counter);
    }
